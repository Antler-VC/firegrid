...
import { DB_ROOT_FORMS, DB_ROOT_LISTS } from 'constants/firegrid';
  ...
  async (values: Record<string, any>) => {
    ...
    if (values._starterTemplateUsed && values._starterTemplate) {
      const templateDocRef = await db
        .doc(DB_ROOT_FORMS + '/' + values._starterTemplate)
        .get();
    ...
    if (values.id) await db.collection(DB_ROOT_FORMS).doc(values.id).set(data);

    const newDocRef = values.id
      ? db.collection(DB_ROOT_FORMS).doc(values.id)
      : await db.collection(DB_ROOT_FORMS).add(data);

    history.push(routes.fieldEditor + '/' + newDocRef.id);
  };

export const _deleteForm =
  (history: RouteComponentProps['history']) => async (id: string) => {
    if (!id) return;

    await db.collection(DB_ROOT_FORMS).doc(id).delete();
    window.location.href = routes.fieldEditor;
  };

export const _newList =
  (
    currentUser: firebase.default.User,
    history: RouteComponentProps['history']
  ) =>
  async (values: Record<string, any>) => {
    ...
    if (values._starterTemplateUsed && values._starterTemplate) {
      const templateDocRef = await db
        .doc(DB_ROOT_LISTS + '/' + values._starterTemplate)
        .get();
      const templateDoc = templateDocRef.data() ?? {};

      if (Array.isArray(templateDoc.values)) data.values = templateDoc.values;
    }

    // Use `id` filed in `values`
    if (values.id) await db.collection(DB_ROOT_LISTS).doc(values.id).set(data);

    const newDocRef = values.id
      ? db.collection(DB_ROOT_LISTS).doc(values.id)
      : await db.collection(DB_ROOT_LISTS).add(data);

    history.push(routes.listEditor + '/' + newDocRef.id);
  };

export const _deleteList =
  (history: RouteComponentProps['history']) => async (id: string) => {
    if (!id) return;

    await db.collection(DB_ROOT_LISTS).doc(id).delete();
    window.location.href = routes.listEditor;
  };
